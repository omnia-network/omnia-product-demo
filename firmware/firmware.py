...
def bt_irq(event, data):
    if event == _IRQ_SCAN_RESULT:
        ...
        ble_result[0] += str(rssi)+'+'+str(adv_data.decode())+","
        #ble.gap_scan(None)
...
def run(sock):
    ...
    async def recv(reader):
        ...
        while True:
            ...
            if(msg_type == 'f'): # read NFC
                ...
                audio_out[0] = I2S(I2S.NUM1,                                  # create I2S peripheral to write audio
                            bck=Pin(bck_pin), ws=Pin(ws_pin), sdout=Pin(sdout_pin),   # sample data to an Adafruit I2S Amplifier
                ...
                if audio_out[0] != '' and len(sound) > 0:
                    num_bytes_written = audio_out[0].write(sound)
                ...
                if flag:
                    if ble[0] != '':
                        send_type[0]=4
                        loop.create_task(scan_ble(ble[0]))
                else:
                    send_type[0]=0
                    ble[0].active(0)
                    ble[0] = ''
    ...
    async def send(writer):
        ...
        while True:
            print(send_type)
            msg = 0
            if(send_type[0]==0 and len(pin_in)>0):
                for p in pin_in:
